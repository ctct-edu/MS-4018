...
2. スライドの左上に表示されるCopilot アイコン（流れ星状のアイコン）をクリックして、**ファイルから新しいプレゼンテーションを作成**をクリックします。
...
4. Copilot はスライドのアウトライン（各スライドに含める見出し案や箇条書き）を表示します。
   ※**プレゼンテーションスタイル**と**画像のユーザー設定**については任意です。
   提案されたトピックを確認し、「新しいプレゼンテーションを生成」をクリックして続行します。
   Copilot は各スライドと内容を生成し、結果一覧を表示します。
...
2. リボンメニューにある右上のCopilotボタンをクリックしてCopilotペインを表示させて、モードをチャットのみに変えます。チャットに以下のプロンプトを入力します。
...
4. 提案された内容の中で気に入ったものを選び、それを使ってスライドを作るように依頼します。

   左上に表示されているCopilotアイコンから**スライドの追加**をクリックします。
   以下のようにプロンプトを入力します（冒頭の部分を一部書き換えてください）。
...
2. 右側のCopilotペインで以下のようにプロンプトを入力します。
